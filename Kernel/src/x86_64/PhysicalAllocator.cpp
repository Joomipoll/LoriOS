...
namespace Memory
{
  ...
  __attribute__((always_inline)) inline void ClearBit(uint64_t bit) { physicalMemoryBitmap[bit >> 5] &= (~(1 << (bit & 31))); }
  __attribute__((always_inline)) inline bool TestBit(uint64_t bit) { return physicalMemoryBitmap[bit >> 5] & (1 << (bit & 31)); }

  uint64_t GetFirstFreeMemoryBlock() 
  {
    if (nextChunk == 0) { nextChunk = 1; }
    ...
    for (uint32_t i = 1; i < (maxPhysicalBlocks >> 5); i++)
    {
        if (physicalMemoryBitmap[i] != 0xffffffff)
        {
            for (uint32_t j = 0; j < 32; j++)
...
// Marks a region in physical memory as being free
  ...
  void FreePhysicalMemoryBlock(uint64_t addr)
  {
    ...
    if (chunk < nextChunk){ nextChunk = chunk; }
  }
