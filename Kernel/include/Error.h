...
struct ErrorOr final
{
    ...
    ALWAYS_INLINE ErrorOr(T val) : m_hasData(true)
    {
        new (data) T(std::move(val));
    }

    ALWAYS_INLINE ErrorOr(Error val) : err(val), m_hasError(true) {}

    ALWAYS_INLINE bool HasError() const { return m_hasError; }
    ...
    ALWAYS_INLINE operator bool() { return !m_hasError; }
