...
namespace fs
{
    ...
    Ext2::Ext2Volume::Ext2Volume(FsNode* device, const char* name)
    {
        ...
        if (super.revLevel)
        {
            ...
            if ((superext.featuresIncompat & (~EXT2_INCOMPAT_FEATURE_SUPPORT)) != 0 || (superext.featuresRoCompat & (~EXT2_READONLY_FEATURE_SUPPORT)) != 0)
            {
                Log::Error(
                    "[Ext2] Incompatible Ext2 features present (Incompt: %x, Read-only Compt: %x). Will not mount volume.", superext.featuresIncompat, superext.featuresRoCompat);
        ...
        blockGroupCount = (super.blockCount % super.blocksPerGroup) ? (super.blockCount / super.blocksPerGroup + 1) : (super.blockCount / super.blocksPerGroup);
        uint32_t secondBlockGroupCount = (super.inodeCount % super.inodesPerGroup) ? (super.inodeCount / super.inodesPerGroup + 1) : (super.inodeCount / super.inodesPerGroup);
        ...
        uint64_t blockGroupOffset = BlockToLocation(LocationToBlock(EXT2_SUPERBLOCK_LOCATION) + 1);
    ...
    uint32_t Ext2::Ext2Volume::GetInodeBlock(uint32_t index, ext2_inode_t& ino)
    {
        ...
        assert(index < triplyIndirectStart + blocksPerPointer * blocksPerPointer * blocksPerPointer);
    ...
    Vector<uint32_t> Ext2::Ext2Volume::GetInodeBlocks(uint32_t index, uint32_t count, ext2_inode_t& ino)
    {
        ...
        assert(index + count < triplyIndirectStart + blocksPerPointer * blocksPerPointer * blocksPerPointer);
        ...
        for(unsigned i = 0; i < blockGroupCount; i++) 
        {
            ...
            for(; e < blocksize / sizeof(uint8_t); e++)
            {
                ...
                for(uint8_t b = 0; b < sizeof(uint8_t) * 8; b++)
                {
                    ...
                    {
                        bitmap[e] |= (1U << b);
                        block = (i * super.blocksPerGroup) + e * (sizeof(uint8_t) * 8) + b;
                        break;
                    }
                }
        ...
        for(unsigned i = 0; i < blockGroupCount; i++)
        {
            ...
            for(unsigned e = 0; e < blocksize / sizeof(uint8_t); e++)
            {
                ...
                for(uint8_t b = 0; b < sizeof(uint8_t) * 8; b++)
                {
                    if(((bitmap[e] >> b) & 0x1) == 0)
                    {
                        inode = (i * super.inodesPerGroup) + e * (sizeof(uint8_t) * 8) + b + 1;
        ...
        for(unsigned i = 0; i < entries.get_length(); i++)
        {
            ...
            if(i == lastIndex)
            {
                ...
                if(strlen(entries[i + 1].name) + sizeof(ext2_directory_entry_t) + blockOffset > blocksize)
                {
                    ...
                    {
                        memset(((uint8_t*)e2dirent) + e2dirent->recordLength, 0, 4 - (e2dirent->recordLength % 4));
    ...
    FsNode* Ext2::Ext2Volume::FindDir(Ext2Node* node, const char* name)
    {
        ...
        { // Could not locate inode in cache
    ...
    ssize_t Ext2::Ext2Volume::Read(Ext2Node* node, size_t offset, size_t size, uint8_t* buffer)
    {
        ...
        */
        ...
        if(readtv2 - readtv1 + (blktv2 - blktv1) > 1200)
            Log::Info("[Ext2] Retrieving inode blocks took %d ms, Reading inode blocks took %d ms", blktv2 - blktv1, readtv2 - readtv1);
    #endif
    ...
    ssize_t Ext2::Ext2Volume::ReadLink(Ext2Node* node, char* pathBuffer, size_t bufSize)
    {
        ...
        if(node->size <= 60)
        {
            ...
            strncpy(pathBuffer, (const char*)node->e2inode.blocks, copySize);
