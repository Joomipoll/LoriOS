...
long SysChmod(RegisterContext* r) 
{
    ...
    size_t pathLen = 0;
    if (strlenSafe(path, pathLen, proc->addressSpace)) { return -EFAULT; }
    if (pathLen > PATH_MAX) { return -ENAMETOOLONG; }
...
long SysReadDir(RegisterContext* r) 
{
    ...
    fs_dirent_t* direntPointer = (fs_dirent_t*)SC_ARG1(r);
    if (!Memory::CheckUsermodePointer(SC_ARG1(r), sizeof(fs_dirent_t), Scheduler::GetCurrentProcess()->addressSpace)) { return -EFAULT; }
...
long SysPRead(RegisterContext* r) 
{
    ...
    if (!Memory::CheckUsermodePointer(SC_ARG1(r), SC_ARG2(r), currentProcess->addressSpace)) { return -EFAULT; }
...
long SysDup(RegisterContext* r) 
{
    ...
    if (fd == requestedFd) { return -EINVAL; }
    ...
    if (flags & O_CLOEXEC) { handle.closeOnExec = true; }
...
long SysSelect(RegisterContext* r) 
{
    ...
    if (evCount) { return evCount; }

    long timeoutUs = 0;
    if (timeout) { timeoutUs = (timeout->tv_sec * 1000000) + (timeout->tv_nsec / 1000); }

    if (timeoutUs) 
    {
        if (fsWatcher.WaitTimeout(timeoutUs)) { return -EINTR; }
    } else {
        if (fsWatcher.Wait()) { return -EINTR; }
    }
