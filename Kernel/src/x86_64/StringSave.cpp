...
long strlenSafe(const char* str, size_t& size, AddressSpace* aSpace) 
{
    ...
    if (!Memory::CheckUsermodePointer(reinterpret_cast<uintptr_t>(str), pageBoundary, aSpace)) { return 1; }
    ...
    for (;;) 
    {
        if (!Memory::CheckUsermodePointer(reinterpret_cast<uintptr_t>(str) + i, PAGE_SIZE_4K, aSpace)) { return 1; }
