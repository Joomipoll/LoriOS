...
void KBHandler(void*, RegisterContext* r) 
{
    ...
    if(keyCode == 0) { keyCode = scancode2Keymap[key]; }
    if(keyWasReleased) { /* Set high bit if key was released */ keyCode |= 0x80; }
    ...
    if (keyQueueEnd >= KEY_QUEUE_SIZE) { keyQueueEnd = 0; }
...
void MouseHandler(void*, RegisterContext* regs) 
{
    ...
    case 3: {
        if (hasScrollWheel) { mouseData[3] = inportb(PS2_DATA); }
        mouseCycle = 0;
        ...
        // Set sign bit according to mouse packet
        ...
        if (packetQueueEnd >= PACKET_QUEUE_SIZE) { packetQueueEnd = 0; }
...
public:
    ...
    ssize_t Read(size_t offset, size_t size, uint8_t* buffer) 
    {
        ...
        *pkt = packetQueue[packetQueueStart];
        ...
        if (packetQueueStart >= PACKET_QUEUE_SIZE) { packetQueueStart = 0; }
 ...
 * They are encoded in resolution commands with a leading scale command
 */
...
namespace PS2 
{
  ...
  void Initialize() 
  {
    ...
    while (inportb(PS2_CMD) & 1) { inportb(PS2_DATA); /* Discard any data */ }
    ...
    // Enable interrupts, enable keyboard and mouse clock disable scancode translation
    ...
    // Set the sample rate a bunch of times to enable scroll wheel
