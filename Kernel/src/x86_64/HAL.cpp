...
namespace HAL 
{
  ...
  { // Always call this first
    ...
    Serial::Initialize();
    Serial::Write("Initializing Lori...\r\n");
  ...
  void InitVideo() 
  {
    ...
    Video::DrawString("Starting Lori ...", 0, 0, 255, 255, 255);
  ...
  void InitExtra() 
  {
    ...
    {
        KernelPanic("CPU does not support SSE4.2, LoriBird OS requires x86_64-v2 support!");
    }

    if (!(cpuidInfo.features_ecx & CPUID_ECX_POPCNT)) 
    {
        KernelPanic("CPU does not support POPCNT, LoriBird OS requires x86_64-v2 support!");
    }

    if (!(cpuidInfo.features_ecx & CPUID_ECX_CX16)) 
    {
        KernelPanic("CPU does not support CMPXCHG16, LoriBird OS requires x86_64-v2 support!");
    }
    Log::Write("OK");
    ...
    while (tagPhys) 
    {
        ...
        switch (tag->id) 
        {
          ...
          case Stivale2TagFramebufferInfo: {
            ...
            if (fbTag->memoryModel == 1) { videoMode.type = VideoModeRGB; }
            break;
          }
