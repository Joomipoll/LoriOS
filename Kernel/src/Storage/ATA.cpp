...
namespace ATA
{
    ...
    inline void WriteControlRegister(uint8_t port, uint8_t reg, uint8_t value) { outportb((port ? controlPort1 : controlPort0) + reg, value); }
    inline uint8_t ReadControlRegister(uint8_t port, uint8_t reg) { return inportb((port ? controlPort1 : controlPort0) + reg); }
...
c1:
    ...
    void IRQHandler(RegisterContext* r) { Log::Info("Disk IRQ"); }
    ...
    int Access(ATADiskDevice* drive, uint64_t lba, uint16_t count, bool write)
    {
        ...
        outportb(busMasterPort + ATA_BMR_STATUS, inportb(busMasterPort + ATA_BMR_STATUS) | 4 | 2);
        ...
        if (write)
            WriteRegister(drive->port, ATA_REGISTER_COMMAND, 0x35);
        else
            WriteRegister(drive->port, ATA_REGISTER_COMMAND, 0x25);
