...
ssize_t PTYDevice::Read(size_t offset, size_t size, uint8_t* buffer)
{
    ...
    if (pty && device == PTYSlaveDevice) { return pty->SlaveRead((char*)buffer, size); } else if (pty && device == PTYMasterDevice) {
...
{
    if (device == PTYMasterDevice) { pty->WatchMaster(watcher, events); } else if (device == PTYSlaveDevice) {
...
{
    if (device == PTYMasterDevice) { pty->UnwatchMaster(watcher); } else if (device == PTYSlaveDevice) {
...
{
    if (device == PTYMasterDevice) { return !!pty->master.bufferPos; } else if (device == PTYSlaveDevice) {
        if (pty->IsCanonical())
