...
void InitVideo() {
    ...
    Video::DrawString("Starting Lori x64...", 0, 0, 255, 255, 255);
...
void InitExtra() {
    ...
    if (!(cpuidInfo.features_ecx & CPUID_ECX_SSE4_2)) {
        KernelPanic("CPU does not support SSE4.2, Lori OS requires x86_64-v2 support!");
    }

    if (!(cpuidInfo.features_ecx & CPUID_ECX_POPCNT)) {
        KernelPanic("CPU does not support POPCNT, Lori OS requires x86_64-v2 support!");
    }

    if (!(cpuidInfo.features_ecx & CPUID_ECX_CX16)) {
        KernelPanic("CPU does not support CMPXCHG16, Lori OS requires x86_64-v2 support!");
    }
    Log::Write("OK");
