...
class Socket : public FsNode
{
  protected:
    int type = 0;
    int domain = 0;
    Semaphore pendingConnections = Semaphore(CONNECTION_BACKLOG);
    List<Socket*> pending;
...
class TCPSocket final : public IPSocket
{
  ...
  protected:
    ...
    int Synchronize(uint32_t seqNumber);
    int Acknowledge(uint32_t ackNumber);
    int SynchronizeAcknowledge(
        uint32_t seqNumber,
        uint32_t ackNumber);
    int Finish();
    int FinishAcknowledge(uint32_t ackNumber);
    int Reset();
    ...
    uint32_t m_lastAcknowledged;
    uint32_t m_remoteSequenceNumber;

    lock_t m_unacknowledgedPacketsLock = 0;
    List<TCPPacket> m_unacknowledgedPackets;
