...
namespace Memory 
{
  ...
  pdpt_t kernelPDPT __attribute__((aligned(4096)));
  ...
  page_table_t AllocatePageTable() 
  {
    ...
    for (int i = 0; i < PAGES_PER_TABLE; i++) { ((page_t*)virt)[i] = 0; }
    assert(pTable.virt);
  ...
  void InitializeVirtualMemory()
  {
    ...
    for (int j = 0; j < TABLES_PER_DIR; j++) { kernelDir[j] = (PAGE_SIZE_2M * j) | 0x83; }
  ...
  void DestroyPageMap(PageMap* pageMap) 
  {
    ...
    {
        if (!pageMap->pageDirs[i]) { continue; }

        if (pageMap->pageDirsPhys[i] < PHYSALLOC_BLOCK_SIZE) { continue; }
        ...
        for (int j = 0; j < TABLES_PER_DIR; j++) 
        {
            ...
            if (dirEnt & PAGE_PRESENT) 
            {
                ...
                if (phys < PHYSALLOC_BLOCK_SIZE) { continue; }
        ...
        if (pageMap->pageTables[i]) { kfree(pageMap->pageTables[i]); }
  ...
  bool CheckKernelPointer(uintptr_t addr, uint64_t len) 
  {
    ...
    if (PML4_GET_INDEX(addr) != PML4_GET_INDEX(KERNEL_VIRTUAL_BASE)) { return 0; }

    if (!(kernelPDPT[PDPT_GET_INDEX(addr)] & 0x1)) { return 0; }
    ...
    if (PDPT_GET_INDEX(addr) == KERNEL_HEAP_PDPT_INDEX) 
    {
        ...
        {
            if (!(kernelHeapDirTables[PAGE_DIR_GET_INDEX(addr)][PAGE_TABLE_GET_INDEX(addr)] & 0x1)) { return 0; }
        }
    } else if (PDPT_GET_INDEX(addr) == PDPT_GET_INDEX(KERNEL_VIRTUAL_BASE)) {
        if (!(kernelDir[PAGE_DIR_GET_INDEX(addr)] & 0x1)) { return 0; }
    } else { return 0; }
  ...
  void* Allocate4KPages(uint64_t amount, PageMap* pageMap) 
  {
    ...
    for (int d = 0; d < 512; d++) 
    {
        ...
        if (!(pageMap->pdpt[d] & 0x1)) { continue; }
  ...
  void PageFaultHandler(void*, RegisterContext* regs)
  {
    ...
    if (process) 
    {
        ...
        if (faultRegion &&
            ...
            {
                if (vmo->refCount <= 1) 
                { // Last reference, no need to clone
