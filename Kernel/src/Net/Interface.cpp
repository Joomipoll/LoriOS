...
namespace Network
{
	...
	void OnReceiveARP(void* data, size_t length)
	{
		...
		{
			IF_DEBUG(debugLevelNetwork >= DebugLevelVerbose, { Log::Warning("[Network] [ARP] Discarding packet (too short)"); });
		...
		{
			NetworkAdapter* adapter = NetFS::GetInstance()->FindAdapter(arp->destPrAddr.value);
    ...
    void OnReceiveIPv4(void* data, size_t length)
    {
		...
		if(checksum.value != CaclulateChecksum(data, sizeof(IPv4Header)).value)
		{
    ...
    int SendIPv4(void* data, size_t length, IPv4Address& source, IPv4Address& destination, uint8_t protocol, NetworkAdapter* adapter)
    {
		...
		ipHeader->version = 4; // Internet Protocol v4
