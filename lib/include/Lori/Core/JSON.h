...
namespace Lori
{
    ...
    struct JSONValue
    {
        ...
        union
        {
            ...
            std::map<JSON_Key, JSONValue>* object;
        ...
        JSONValue(std::map<JSON_Key, JSONValue>&& o)
        {
            type = TypeObject;

            data.object = new std::map<JSON_Key, JSONValue>(std::move(o));
        }
        ...
        static JSONValue NewObject()
        {
            JSONValue v;
            v.type = TypeObject;
            v.data.object = new std::map<JSON_Key, JSONValue>();
        ...
        void ReleaseData()
        {
            if(type == TypeString)      delete data.str;
            else if(type == TypeArray)  delete data.array;
            else if(type == TypeObject) delete data.object;
        }
        ...
        {
            if(traits.isFloatingPoint)  return static_cast<I>(data.dbl);
            else                        return static_cast<I>(data.sLong);
        }
