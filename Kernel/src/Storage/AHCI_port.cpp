...
namespace AHCI
{
  ...
  Port::Port(int num, hba_port_t* portStructure, hba_mem_t* hbaMem)
  {
    ...
    {
      case 0:
        Log::Error("[AHCI] Disk has a corrupted or non-existant GPT. Master Boot Record disks are NOT supported.");
        break;
      case -1:
  ...
  int Port::ReadDiskBlock(uint64_t lba, uint32_t count, void* _buffer)
  {
    ...
    while (blockCount >= 8 && count)
    {
        ...
        if (int e = Access(lba, 8, physBuffers[buf], 0); e)
    ...
    while (blockCount >= 2 && count)
    {
        ...
        if (int e = Access(lba, 2, physBuffers[buf], 0); e)
        {
            ReleaseBuffer(buf);
            return e;
        }
    ...
    while (blockCount && count)
    {
        ...
        if (int e = Access(lba, 1, physBuffers[buf], 0); e)
        {
            ReleaseBuffer(buf);
            return e;
        }
  ...
  int Port::WriteDiskBlock(uint64_t lba, uint32_t count, void* _buffer)
  {
    ...
    while (blockCount-- && count)
    {
        ...
        if (int e = Access(lba, 1, physBuffers[buf], 1); e)
        {
            ReleaseBuffer(buf);
            return e;
        }
  ...
  int Port::Access(uint64_t lba, uint32_t count, uintptr_t physBuffer, int write)
  {
    ...
    commandTable->prdt_entry[0].dbc = 512 * count - 1;
  ...
  void StopCMD(hba_port_t* port)
  {
    ...
    if (spin <= 0)
        Log::Warning("[SATA] StopCMD stuck!");
