...
DirectoryEntry::DirectoryEntry(FileSystemNode* node, const char* name) : node(node)
{
...
namespace FileSystem
{
    ...
    class Root : public FileSystemNode {
    public:
        Root()
        {
            inode = 0;
    ...
    FileSystemNode* GetRoot() { return &root; }
    ...
    FileSystemNode* FollowLink(FileSystemNode* link, FileSystemNode* workingDir)
    {
        ...
        FileSystemNode* node = ResolvePath(buffer, workingDir, false);
    ...
    FileSystemNode* ResolvePath(const String& path, const char* workingDir, bool followSymlinks)
    {
        if(path.Empty())
        {
            Log::Warning("FileSystem::ResolvePath: path is empty!");
            return nullptr;
        }
        ...
        { // If the path starts with '/' then treat as an absolute path
            FileSystemNode* wdNode = ResolvePath(workingDir, (FileSystemNode*)nullptr);
            assert(wdNode);
            return ResolvePath(path, wdNode, followSymlinks);
        } else {
            return ResolvePath(path, (FileSystemNode*)nullptr, followSymlinks);
        }
    }

    FileSystemNode* ResolvePath(const String& path, FileSystemNode* workingDir, bool followSymlinks)
    {
        if(path.Empty())
        {
            Log::Warning("FileSystem::ResolvePath: path is empty!");
        ...
        for(; i < components.size() - 1; i++)
        {
            ...
            FileSystemNode* node = FileSystem::FindDir(currentNode, component.c_str());
            if (!node)
            {
        ...
        FileSystemNode* node = FileSystem::FindDir(currentNode, components[i].c_str());
        if(!node)
        {
        ...
        size_t amountOfSymlinks = 0;
        while(followSymlinks && node->IsSymlink())
        { // Check for symlinks
            if (amountOfSymlinks++ > MAXIMUM_SYMLINK_AMOUNT)
            {
    ...
    FileSystemNode* ResolveParent(const char* path, const char* workingDir)
    {
        ...
        FileSystemNode* parentDirectory = nullptr;

        if (dirPath == nullptr)
        {
            parentDirectory = FileSystem::ResolvePath(workingDir);
        } else {
            *(dirPath - 1) = 0;      // Cut off the directory name from the path copy
            ...
            { // Root
                ...
                parentDirectory = FileSystem::ResolvePath(pathCopy, workingDir);
            }
        }
    ...
    FileSystemNode* ResolveParent(const char* path, FileSystemNode* workingDir)
    {
        char pathCopy[strlen(path) + 1];
        strcpy(pathCopy, path);
    ...
    FileSystemNode* Root::FindDir(const char* name)
    {
        if (strcmp(name, ".") == 0)
            return this;
        if (strcmp(name, "..") == 0)
    ...
    ErrorOr<UNIXOpenFile*> Open(FileSystemNode* node, uint32_t flags) { return node->Open(flags); }

    int Link(FileSystemNode* dir, FileSystemNode* link, DirectoryEntry* ent)
    {
        assert(dir);
        assert(link);
    ...
    int Unlink(FileSystemNode* dir, DirectoryEntry* ent, bool unlinkDirectories)
    {
        assert(dir);
        assert(ent);
    ...
    void Close(FileSystemNode* node) { return node->Close(); }
    ...
    int Rename(FileSystemNode* olddir, const char* oldpath, FileSystemNode* newdir, const char* newpath)
    {
        ...
        FileSystemNode* oldnode = ResolvePath(oldpath, olddir);
        ...
        FileSystemNode* newpathParent = ResolveParent(newpath, newdir);
        ...
        FileSystemNode* newnode = ResolvePath(newpath, newdir);
        ...
        strncpy(oldpathDirent.name, FileSystem::BaseName(oldpath).c_str(), NAME_MAX);

        DirectoryEntry newpathDirent;
        strncpy(newpathDirent.name, FileSystem::BaseName(newpath).c_str(), NAME_MAX);

        if ((oldnode->flags & FS_NODE_TYPE) != FS_NODE_SYMLINK &&
            oldnode->volumeID == newpathParent->volumeID)
            { // Easy shit we can just link and unlink
                FileSystemNode* oldpathParent = FileSystem::ResolveParent(oldpath, olddir);
