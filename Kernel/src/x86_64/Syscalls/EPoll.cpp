...
long SysEpollWait(RegisterContext* r) 
{
    ...
    auto epollToPollEvents = [](uint32_t ep) -> int {
        ...
        if (ep & EPOLLIN) { evs |= POLLIN; }
        if (ep & EPOLLOUT) { evs |= POLLOUT; }
        if (ep & EPOLLHUP) { evs |= POLLHUP; }
        if (ep & EPOLLRDHUP) { evs |= POLLRDHUP; }
        if (ep & EPOLLERR) { evs |= POLLERR; }
        if (ep & EPOLLPRI) { evs |= POLLPRI; }
        return evs;
    };
