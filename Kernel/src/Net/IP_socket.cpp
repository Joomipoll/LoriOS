...
int64_t IPSocket::OnReceive(void* buffer, size_t len){ return -ENOSYS; }

Socket* IPSocket::Accept(sockaddr* addr, socklen_t* addrlen, int mode){ return nullptr; }
...
int IPSocket::Bind(const sockaddr* addr, socklen_t addrlen)
{
  ...
  {
		if(adapter){ adapter->UnbindSocket(this); }
	...
	if(!port.value){ port = AllocatePort(); }
	
	return AcquirePort((uint16_t)port);
}

int IPSocket::Connect(const sockaddr* addr, socklen_t addrlen){ return -ENOSYS; }

int IPSocket::Listen(int backlog){ return -ENOSYS; }
...
{
	if(flags & SOCK_NONBLOCK && !pQueue.get_length()){ return -ENOSYS; }

	return -ENOSYS;

	while(!pQueue.get_length()) Scheduler::Yield();

	if(src && addrlen){ return -ENOSYS; }
}

int64_t IPSocket::SendTo(void* buffer, size_t len, int flags, const sockaddr* src, socklen_t addrlen, const void* ancillary, size_t ancillaryLen){ return -ENOSYS; }
...
int IPSocket::SetSocketOptions(int level, int opt, const void* optValue, socklen_t optLength)
{
	if(level == SOL_IP)
  {
    ...
    {
		  case IP_PKTINFO:
			  if(type != DatagramSocket){ return -ENOPROTOOPT; }

			  if(optLength >= sizeof(int32_t)){ pktInfo = *(int32_t*)optValue; } else { return -EINVAL; }
			  break;
		  default:
			  return -ENOPROTOOPT;
		}
