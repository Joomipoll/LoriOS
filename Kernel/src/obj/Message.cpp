...
MessageEndpoint::MessageEndpoint(uint16_t maxSize)
{
    ...
    messageQueueLimit = 0x300000 / maxMessageSize; // 3MB
...
MessageEndpoint::~MessageEndpoint(){ Destroy(); }
...
int64_t MessageEndpoint::Call(uint64_t id, uint16_t size, uint64_t data, uint64_t rID, uint16_t* rSize, uint8_t* rData, int64_t timeout)
{
    ...
    if(size > maxMessageSize){ return -EINVAL; }
...
int64_t MessageEndpoint::Write(uint64_t id, uint16_t size, uint64_t data)
{
    ...
    if(queueAvailablilitySemaphore.Wait()){ return -EINTR; }

    acquireLock(&peer->queueLock);

    Message* m;
    if(peer->cache.Dequeue(m))
    { // Check for a cached message allocaiton
        m->size = size;
        m->id = id;
    ...
    while(peer->waiting.get_length() > 0){ peer->waiting.remove_at(0)->Signal(); }
