...
int Socket::Listen(int backlog) { return -EOPNOTSUPP; /* If Listen has not been implemented assume that the socket is not connection-oriented */ }

ssize_t Socket::Read(size_t offset, size_t size, uint8_t* buffer) { return Receive(buffer, size, 0); }

int64_t Socket::Receive(void* buffer, size_t len, int flags) { return ReceiveFrom(buffer, len, flags, nullptr, nullptr); }
...
int LocalSocket::Connect(const sockaddr* addr, socklen_t addrlen)
{
    ...
    if (!(sock && sock->IsListening()))
    { // Make sure the socket is both present and listening
...
int64_t LocalSocket::ReceiveFrom(void* buffer, size_t len, int flags, sockaddr* src, socklen_t* addrlen,
    ...
    if (inbound->Empty() && (flags & MSG_DONTWAIT)) { return -EAGAIN; } else {
        while (inbound->Empty())
            inbound->Wait();
    }
    if (flags & MSG_PEEK) { return inbound->Peek(buffer, len); } else {
...
int64_t LocalSocket::SendTo(void* buffer, size_t len, int flags, const sockaddr* src, socklen_t addrlen,
    ...
    if (type == StreamSocket && (flags & MSG_DONTWAIT) && outbound->Pos() + len >= STREAM_MAX_BUFSIZE) { return -EAGAIN; } else {
