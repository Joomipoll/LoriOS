...
namespace AHCI
{
    ...
    int Init()
    {
        ...
        ahciBaseAddress = controllerPCIDevice->GetBaseAddressRegister(5);
        ...
        if (debugLevelAHCI >= DebugLevelNormal) {
            Log::Info("[AHCI] Interrupt Vector: %x, Base Address: %x, Virtual Base Address: %x", -1, ahciBaseAddress, ahciVirtualAddress);
