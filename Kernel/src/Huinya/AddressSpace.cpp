...
bool AddressSpace::RangeInRegion(uintptr_t base, size_t size)
{
    ...
    for (; it != m_regions.end(); it++)
    {
        if (base < it->Base())
        {
            ...
            return false;
        } else if (base >= it->Base() && base < it->End()) { base = it->End(); }
...
MappedRegion* AddressSpace::FindAvailableRegion(size_t size)
{
    ...
    for (; it != m_regions.end(); it++)
    {
        ...
        if (base >= it->Base() && base < it->End())
            base = it->End();
            end = base + size;

        if (end > it->Base() && end <= it->End())
            base = it->End();
            end = base + size;

        if (base < it->Base() && end > it->End())
            base = it->End();
            end = base + size;
    }
