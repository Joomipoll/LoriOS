...
int LoadModuleSegments(Module* module, FsNode* file, elf64_header_t& header) {
    ...
    assert(moduleInfoIndex < symCount && symbols[moduleInfoIndex].size == sizeof(LoriModuleInfo));
    ...
    LoriModuleInfo* moduleInfo = reinterpret_cast<LoriModuleInfo*>(moduleInfoSection.addr + moduleInfoSymbol.value);
