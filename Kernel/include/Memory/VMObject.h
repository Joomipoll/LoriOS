...
struct MappedRegion
{
    ...
    MappedRegion() = delete;
    ALWAYS_INLINE MappedRegion(uintptr_t base, size_t size) : base(base), size(size), vmObject(nullptr) { }

    ALWAYS_INLINE MappedRegion(uintptr_t base, size_t size, FancyRefPtr<VMObject> vmObject) : base(base), size(size), vmObject(vmObject) { }
