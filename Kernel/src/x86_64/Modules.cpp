...
{
    if (name) { kfree(name); }
    if (description) { kfree(description); }
}
...
namespace ModuleManager
{
  ...
  int LoadModuleSegments(Module* module, FileSystemNode* file, elf64_header_t& header)
  {
    ...
    ELF64Section* shStrTab = nullptr;
    ELF64Section* symStrTab = nullptr;
    ELF64Section* symTab = nullptr;
    ...
    for (unsigned i = 0; i < shNum; i++)
    {
        ...
        {
            if (i == shStrIndex) { shStrTab = &sections[i]; } else if (sections[i].type == SHT_SYMTAB) {
              symTab = &sections[i];
            }
        }
    }
    ...
    assert(FileSystem::Read(file, symTab->off, symTab->size, symbols) == symTab->size);
    {
        for (unsigned i = 0; i < symCount; i++)
        {
            ...
            if (symbol.name > symStrTab->size || !symbol.name) { continue; }

            uint8_t symbolType = ELF64_SYM_TYPE(symbol.info);
            uint8_t symbolBinding = ELF64_SYM_BIND(symbol.info);
            if (symbolType == STT_FILE || symbolType == STT_SECTION) { continue; } else if (symbolType != STT_OBJECT && symbolType != STT_FUNC && symbolType != STT_NOTYPE)
            {
            ...
            if (!strcmp(symStringTable + symbol.name, "_moduleInfo")) { moduleInfoIndex = i; }
    ...
    for (int i = 0; i < shNum; i++)
    {
        ...
        {
            assert(section.align <= PAGE_SIZE_4K);

            ModuleSegment moduleSegment;
            moduleSegment.write = !!(section.flags & SHF_WRITE);

            uint64_t pageAttributes = PAGE_PRESENT;
            if (moduleSegment.write) { pageAttributes |= PAGE_WRITABLE; }
    ...
    for (int i = 0; i < shNum; i++)
    {
        ...
        } else { assert(section.type != SHT_REL); }
    }
    ...
    for (unsigned i = 0; i < symCount; i++)
    {
        ...
        uint8_t symbolType = ELF64_SYM_TYPE(symbol.info);
        uint8_t symbolBinding = ELF64_SYM_BIND(symbol.info);
        if (symbolType == STT_FILE || symbolType == STT_SECTION) { continue; }
