...
namespace ACPI 
{
  ...
  void* FindSDT(const char* signature, int index) 
  {
    ...
    for (int i = 0; i < entries; i++) 
    {
        ...
        if (memcmp(h->signature, signature, 4) == 0 && _index++ == index) { return h; }
    }
  ...
  void Init() 
  {
    if (desc) { goto success; /* Already found */ }

    for (int i = 0; i <= 0x7BFF; i += 16) 
    { // Search first KB for RSDP, the RSDP is aligned on a 16 byte boundary
