...
VMObject::VMObject(size_t size, bool anonymous, bool shared) : size(size), anonymous(anonymous), shared(shared) { assert(!(size & (PAGE_SIZE_4K - 1))); }
...
void PhysicalVMObject::MapAllocatedBlocks(uintptr_t base, PageMap* pMap)
{
    ...
    for(unsigned i = 0; i < (size >> PAGE_SHIFT_4K); i++)
    {
        ...
        if(block)
            ...
            Memory::MapVirtualMemory4K(0, virt, 1, PAGE_USER, pMap);
...
VMObject* PhysicalVMObject::Clone()
{
    ...
    for(unsigned i = 0; i < (size >> PAGE_SHIFT_4K); i++)
    {
        ...
        if(block)
        {
            ...
            Memory::KernelMapVirtualMemory4K(block << PAGE_SHIFT_4K, (uintptr_t)virtBuffer, 1);
...
ProcessImageVMObject::ProcessImageVMObject(uintptr_t base, size_t size, bool write) : PhysicalVMObject(size, false, false), write(write), base(base) { }
...
AnonymousVMObject::AnonymousVMObject(size_t size) : PhysicalVMObject(size, true, false) { }
